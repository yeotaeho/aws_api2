...
public class GoogleController {
     ...
     */
    @GetMapping("/auth-url")
    public ResponseEntity<Map<String, Object>> getGoogleAuthUrl(HttpServletRequest request) {
        // 환경 변수에서 가져오기
        ...
        if (redirectUri == null || redirectUri.isEmpty()) {
            ...
            if ((scheme.equals("http") && serverPort != 80) ||
                ...
                redirectUri = String.format("%s://%s:%d/google/callback", scheme, serverName, serverPort);
            } else {
                redirectUri = String.format("%s://%s/google/callback", scheme, serverName);
            }

            System.out.println("⚠️ GOOGLE_REDIRECT_URI 환경 변수가 설정되지 않아 동적으로 생성: " + redirectUri);
        }
    ...
    public ResponseEntity<?> googleCallback(
        ...
        if (code != null) {
            try {
                ...
                // 4. 구글 OAuth 원본 토큰을 Redis에 저장 (선택적 - 실패해도 계속 진행)
                try {
                ...
                if (jwtRefreshToken != null) {
                    ...
                    // 6. 프론트엔드로 리다이렉트 (Access Token만 URL에 포함)
                ...
                // 상세한 에러 로깅
                System.err.println("========================================");
                System.err.println("❌ 구글 인증 처리 중 오류 발생");
